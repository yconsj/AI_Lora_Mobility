...
class OmnetEnv:
    ...
    def unpack_tools(self):
        toolchain_file = os.path.join(self.omnetpp_root, 'tools',
                                      'opp-tools-win32-x86_64-mingw64-toolchain.7z').replace('\\', '/')
        dependencies_file = os.path.join(self.omnetpp_root, 'tools',
                                         'opp-tools-win32-x86_64-mingw64-dependencies.7z').replace('\\', '/')
        ...
        if os.path.exists(dependencies_file):
            ...
            qtbinpatcher = os.path.join(self.omnetpp_root, 'tools', 'win32.x86_64', 'opt', 'mingw64', 'bin',
                                        'qtbinpatcher.exe').replace('\\', '/')
    ...
    def run_simulation(self, ini_config, batch_size=1):
