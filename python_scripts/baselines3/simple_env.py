...
class SimpleBaseEnv(gym.Env):
    def __init__(self, render_mode="none"):
        ...
        self.received2 = 0
        self.last_packet = 0
        self.pos_reward_max = 0.0001
        self.pos_reward_min = 0.0
        self.pos_penalty_max = 0
        ...
        self.send_interval = 200
        self.node1 = node(pos1, time_to_first_packet=100, send_interval=self.send_interval)
        self.node2 = node(pos2, time_to_first_packet=200, send_interval=self.send_interval)
    ...
    def get_pos_reward(self, pos1, pos2, time_of_packet):
        time_to_packet = (time_of_packet - self.steps)  # time to packet
        # scaled_time = (time_to_packet / self.max_steps)

        # time = (time - self.steps)  # time to packet
        # scaled_time = 1 - (time / self.send_interval)  # positive
        scaled_time = max(0, 1 - abs(time_to_packet / self.send_interval))  # Values in [0, 1]

        distance = abs(pos1 - pos2)
        scaled_distance = distance / self.max_distance  # positive
        # scaled_distance_time = scaled_distance * scaled_time

        # time_distance_abs_diff = abs(scaled_distance - scaled_time)

        time_factor = np.exp(-scaled_time)  # Closer to 1 when time_to_packet = 0, decays as time increases
        weighted_distance = (1 - scaled_distance) * time_factor  # Reward for being close, weighted by time

        # scaled_distance_time = (scaled_distance + scaled_time) / 2
        # Return reward based on scaled distance between a min and max reward
        # reward = self.pos_reward_max - time_distance_abs_diff * (self.pos_reward_max - self.pos_reward_min)
        ...
        reward = self.pos_reward_min + weighted_distance * (self.pos_reward_max - self.pos_reward_min)
    ...
    def step(self, action):
        ...
        #print(f"({self.total_misses = }, {self.total_received = } )")
        done = (self.steps >= self.max_steps) or (self.total_misses > self.total_received) or (self.total_misses >= 20)
