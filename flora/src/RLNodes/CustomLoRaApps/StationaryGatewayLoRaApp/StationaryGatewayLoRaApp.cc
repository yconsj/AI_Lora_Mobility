...
#include "StationaryGatewayLoRaApp.h"
...
Define_Module(StationaryGatewayLoRaApp);


void StationaryGatewayLoRaApp::initialize(int stage)
{
    if (stage == 0) {
    ...
    //EV << "stationary_app parent_module:" << parentModule->getName() << " index: " << parentModule->getIndex() << omnetpp::endl;
}
...
int getLoRaNodeIndex(MacAddress node_mac_address) {
    ...
    for (int node_index = 0; node_index < n_sim_nodes; node_index++) {
...
void StationaryGatewayLoRaApp::startUDP()
{
    EV << "StationaryGatewayLoRaApp: Starting UDP" << endl;
    socket.setOutputGate(gate("socketOut"));
    const char *localAddress = par("localAddress");
    socket.bind(*localAddress ? L3AddressResolver().resolve(localAddress) : L3Address(), localPort);
    EV << "StationaryGatewayLoRaApp: Reached past first resolve" << endl;
    ...
    while ((token = tokenizer.nextToken()) != nullptr) {
        EV << "StationaryGatewayLoRaApp: entering loop" << endl;
        EV << token << endl;
        L3Address result;
...
void StationaryGatewayLoRaApp::handleMessage(cMessage *msg)
{
...
void StationaryGatewayLoRaApp::processLoraMACPacket(Packet *pk)
{
    ...
    stateLogger->logStationaryGatewayPacketReception(parentModule->getIndex(),
            getLoRaNodeIndex(transmitter_mac_address),
            transmitter_sequence_number);
    EV << "stationary_app parent_module:" << parentModule->getName() << omnetpp::endl;
    // packet counter:
...
void StationaryGatewayLoRaApp::sendPacket()
{
...
void StationaryGatewayLoRaApp::receiveSignal(cComponent *source, simsignal_t signalID, intval_t value, cObject *details)
{
    if (simTime() >= getSimulation()->getWarmupPeriod())
        counterOfSentPacketsFromNodes++;
}

void StationaryGatewayLoRaApp::finish()
{
