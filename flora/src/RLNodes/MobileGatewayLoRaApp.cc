...
bool MobileGatewayLoRaApp::isPacketProcessed(const Ptr<const LoRaMacFrame> &pkt)
{
...
void MobileGatewayLoRaApp::updateKnownNodes(Packet* pkt)
{
    const auto & frame = pkt->peekAtFront<LoRaMacFrame>();
...
void MobileGatewayLoRaApp::addPktToProcessingTable(Packet* pkt)
{
    const auto & frame = pkt->peekAtFront<LoRaMacFrame>();
...
void MobileGatewayLoRaApp::processScheduledPacket(cMessage* selfMsg)
{
...
void MobileGatewayLoRaApp::evaluateADR(Packet* pkt, L3Address pickedGateway, double SNIRinGW, double RSSIinGW)
{
    bool sendADR = false;
